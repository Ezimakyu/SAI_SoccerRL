...
def get_phase_targets(episode_time, freq, phase_offset=0.0):
    """Calculates phase-based knee targets."""
...
def calculate_reward(state, next_state, action, prev_action, qpos0, prev_base_ang_vel, 
    ...
    if lean_magnitude > C.LEAN_THRESHOLD: terminated = True; reason = "Fell Over (Tilt)"
    ...
    # [UPDATED] Soft Lock Target (0.1 rads)
    if phase < 0.5:
        r_stance_straight = np.exp(-np.square(current_right_knee - 0.1) / 0.1) * C.STANCE_STRAIGHT_W
    else:
        r_stance_straight = np.exp(-np.square(current_left_knee - 0.1) / 0.1) * C.STANCE_STRAIGHT_W
    ...
    total_reward = (r_alive + r_height + r_posture + r_split + r_hip_splay + r_stance_straight + 
                    r_lin_vel_z + r_ang_vel_xy + r_heading + r_side_drift + r_yaw_spin +
                    r_knee_bend + r_energy + r_action_rate +
                    (r_run * upright_gate) + (r_phase * upright_gate))
